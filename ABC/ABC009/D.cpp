...
VVL mattime(const VVL& A, const VVL& B) {
    VVL R(K, VL(K, 0));
...
VVL calctime(VVL M, ll n) {
    if (n == 1) return M;

    if (n % 2 == 0) {
        return calctime(mattime(M, M), n / 2);
    } else {
        return mattime(calctime(M, n - 1), M);
    }
}

ll vtime(const VVL& M, const VL& A) {
    ll sum = 0;
    REP(i, K) {
        sum ^= (M[0][i] & A[i]);
    }
...
int main() {
    ...
    if (M <= K) {
        ...
        cout << vtime(calctime(C, M - K), A) << endl;
    }
