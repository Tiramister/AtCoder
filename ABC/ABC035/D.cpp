...
int main() {
    ...
    VL tdis(N + 1, INF), rdis(N + 1, INF);
    tdis[0] = rdis[0] = 0;

    priority_queue<PL, V<PL>, greater<PL>> tque, rque;
    ...
    while (!tque.empty()) {
        ...
        ll tv = tp.snd;
        if (tdis[tv] < tp.fst) continue;

        for (auto p : tpath[tv]) {
            ll sv = p.fst;
            ll cost = p.snd;
            if (tdis[sv] > tdis[tv] + cost) {
                tdis[sv] = tdis[tv] + cost;
                tque.push({tdis[sv], sv});
            }
        }
    }
    ...
    while (!rque.empty()) {
        ...
        ll rv = rp.snd;
        if (rdis[rv] < rp.fst) continue;

        for (auto p : rpath[rv]) {
            ll sv = p.fst;
            ll cost = p.snd;
            if (rdis[sv] > rdis[rv] + cost) {
                rdis[sv] = rdis[rv] + cost;
                rque.push({rdis[sv], sv});
            }
        }
    }
    ...
    REP(i, N) {
        if (tdis[i] + rdis[i] <= T) {
            ans = max(ans, A[i] * (T - tdis[i] - rdis[i]));
        }
    }
