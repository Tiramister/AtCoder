...
bool PLCompare(const PL& a, const PL& b) {
  // firstで小さい順ソート
  if (a.fst < b.fst) {
    return true;
  } else {
...
int main() {
  ...
  sort(ALL(red), PLCompare);
  sort(ALL(blue), PLCompare);

  REP(i, N) {
    ll maxid = -1;
    REP(j, N) {
      if (red[j].fst < blue[i].fst) {
        if (!used[j] && (red[j].snd < blue[i].snd)) {
          if (maxid < 0 || red[j].snd > red[maxid].snd) {
            maxid = j;
          }
        }
      } else {
