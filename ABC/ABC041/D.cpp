...
int N, M;
V<bitset<16>> child(16);
VL dp(1 << 16, -1);

/* ------------- Functions -------------- */

// statが使われた状態で残った頂点の並べ方
ll pat(bitset<16> stat) {

    if (dp[stat.to_ullong()] >= 0) {
        return dp[stat.to_ullong()];
    }
    ...
    REP(i, N) {
        if (stat.test(i)) {
            // 使用済み
            continue;
        } else if (((child[i] ^ stat) & child[i]).any()) {
            // 子が残ってるので右端に置けない
            continue;
        } else {
            sum += pat(stat.set(i));
