...
bool arrivable(int dis, const VI& move, int maxd) {
    if (dis % 2 > 0 || dis < 0) {
        return false;
    }
    ...
    VVB dp(SIZE(move) + 1, VB(maxd + 1, false));
    // dp[i][j] = i個目までの移動でjだけ移動できるか
...
int main() {
    ...
    VVI move(2);
    // move[0]がx方向、move[1]がy座標

    int maxd[2] = {0, 0};
    // 各座標について到達しうる最大の絶対値

    int dir = 0, dis = 0, firstx = true;
    for (char c : s) {
        if (c == 'F') {
            dis++;
        } else {
    ...
    REP(i, 2) {
        if (!arrivable(maxd[i] - abs(p[i]), move[i], maxd[i])) {
            cout << "No" << endl;
            return 0;
        }
    }
