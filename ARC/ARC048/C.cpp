...
ll calc_pow(ll b, ll n) {
    if (n == 0) return 1;
    if (n == 1) return b;

    if (n % 2 == 0) {
        return calc_pow(sq(b) % MOD, n / 2) % MOD;
    } else {
        return calc_pow(b, n - 1) * b % MOD;
    }
}
...
int main() {
    ...
    FOR(i, itr + 1, N) {
        if (L[i] < gcd * 2) {
            gcd = calc_gcd(gcd, L[i]);
        }
    }

    ans = (ans * calc_pow(2, (gcd + 1) / 2)) % MOD;
    cout << ans << endl;
    return 0;
}
