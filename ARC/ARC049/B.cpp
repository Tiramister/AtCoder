...
int N;
V<int> x(1010), y(1010), c(1010);

/* ------------- Functions -------------- */

// 時刻tまでに全員が到達可能な点が存在するならtrue
bool arrivable(double t) {
    double lx = x[0] - t / c[0];
    double ux = x[0] + t / c[0];
    double ly = y[0] - t / c[0];
    double uy = y[0] + t / c[0];
    ...
    NREP(i, N - 1) {
        double lxi = x[i] - t / c[i];
        double uxi = x[i] + t / c[i];
        double lyi = y[i] - t / c[i];
        double uyi = y[i] + t / c[i];

        if (lx > uxi || ux < lxi || ly > uyi || uy < lyi) {
            return false;
        } else {
            lx = max(lx, lxi);
            ux = min(ux, uxi);
