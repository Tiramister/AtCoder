...
// 長方形が円を完全に含み得るならtrue
bool includeCircle(int ix, int iy) {

    bool include = true;

    if (ix == 1 && x[ix] > x[0] - r) {
        include = false;
    } else if (ix == 2 && x[ix] < x[0] + r) {
        include = false;
    }

    if (iy == 1 && y[iy] > y[0] - r) {
        include = false;
    } else if (iy == 2 && y[iy] < y[0] + r) {
        include = false;
    }
...
// 頂点が円に含まれるならtrue
bool inCircle(int ix, int iy) {
    ll dis_sq = square(x[ix] - x[0]) + square(y[iy] - y[0]);
    return dis_sq <= square(r);
}
