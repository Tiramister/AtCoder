...
bool effectsort(const PL& a, const PL& b) {
...
int main() {
    ...
    ll Q, H, S, D;
    cin >> Q >> H >> S >> D;
    VPL tea_data = {{Q, 1}, {H, 2}, {S, 4}, {D, 8}};
    sort(ALL(tea_data), effectsort);
    ...
    ll ans = 0;
    REP(i, 4) {
        ll tea_value = tea_data[i].fst;
        ll tea_size = tea_data[i].snd;
        ans += tea_value * (N / tea_size);
        N = N % tea_size;
    }
    cout << ans << endl;
    return 0;
